...
public class TerrainLoadMgr
{
    ...
    public void SetRoot(Transform terrain, Transform collider, string name)
    {
        ...
        // 2 需要场景中有 untime_parts   对象
        parts_root = GameObject.Find("runtime_parts").transform;
        terrain_name = name;
        // 3 找到要加载的场景的数据文件目录，就是分割后的TerrainData
        string path = "Assets/Resources/" + name + "/" + name;
        // 4 这个是加载parts下面的对象信息，包括地形的配置设置数据
...
/// <summary>
/// 加载地形配置数据，包括 parts下的数据信息，此时还未加载每个TerrainData
/// </summary>
...
/// <returns></returns>
    ...
    /// <summary>
    /// 读取所有part下信息，每个对象抽象成了TerrainPart数据对象
    /// </summary>
    /// <param name="reader"></param>
    private void LoadPartsInfo(BinaryReader reader)
    {
        //  Part下的每个对象，抽象成了TerrainPart对象，就是场景对象的数据
    ...
    public void LoadCollider(int xx, int yy)
    {
        ...
        bx.isTrigger = true;
        int size = terrain_info.sliceSize;
        bx.center = new Vector3(size / 2 , size / 2, size / 2);
        bx.size = new Vector3(size, size, size);
        bx.enabled = false;
    ...
    /// <summary>
    /// 根据地形，构建 Bounds，在范围内的parts对象，会被加载
    /// </summary>
    /// <param name="xx"></param>
    /// <param name="yy"></param>
