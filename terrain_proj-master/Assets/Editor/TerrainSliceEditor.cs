...
public class TerrainSliceEditor : Editor
{
    ...
    private static void Slicing()
    {
        ...
        #region 地形整体配置数据的收集，也就是你Terrain组件中设置面板中的数据
        ...
        // AssetDatabase.CreateAsset(mat,savepath+terrain.name);
        // Debug.Log($"创建材质成功:{savepath+terrain.name}");
        ...
        try
        {
            ...
            for (int x = 0; x < terrainsWide; ++x)
            {
                for (int y = 0; y < terrainsLong; ++y)
                {
                    ...
                    EditorUtility.DisplayProgressBar("正在分割地形", terrainName,
                        (float) (x * terrainsWide + y) / (float) (terrainsWide * terrainsLong));
                    ...
                    alphamap = terrainData.GetAlphamaps(x * newData.alphamapWidth, y * newData.alphamapHeight,
    ...
    /// <summary>
    /// 将Terrain的配置信息写入二进制，将parts下面的对象，写入二进制配置文件中
    /// </summary>
    ...
    private static void WriteTerrainInfo(Terrain terrain, string path)
    {
        ...
        // 3 保存parts下面的所有子物体的transform信息和预制件资源 信息， 包括，光照贴图的信息
        WriteParts(writer);
        writer.Flush();
        writer.Close();
        fs.Close();
    ...
    /// <summary>
    /// 保存parts下面的所有子物体的transform信息和预制件资源 信息
    /// 包括，光照贴图的信息
    /// </summary>
